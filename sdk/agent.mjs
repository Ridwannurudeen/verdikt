...
export function createVerdiktAgent({ publicClient, walletClient, account, court }) {
  const read = (functionName, args = []) => publicClient.readContract({ address: court, abi: AGENT_ABI, functionName, args });
  const write = (functionName, args, value = 0n) =>
    walletClient.writeContract({ address: court, abi: AGENT_ABI, functionName, args, value, account, chain: null });
  ...
  const agent = {
    ...
    /// Decentralized keeper sweep: finalize every case that is Ruled and past its appeal window
    /// (or at MAX_ROUND). Returns the cases it settled. `now` overridable for testing.
    async runKeeper({ now = BigInt(Math.floor(Date.now() / 1000)), onAction } = {}) {
      const [next, appealWindow, maxRound] = await Promise.all([read("nextCaseId"), read("appealWindow"), read("MAX_ROUND")]);
      const settled = [];
      for (let id = 1n; id < next; id++) {
        ...
        const windowEnd = c.rulingTime + appealWindow;
    ...
    async bountyFor(bounty, caseId) {
      const k = await publicClient.readContract({ address: bounty, abi: BOUNTY_ABI, functionName: "key", args: [court, BigInt(caseId)] });
      return publicClient.readContract({ address: bounty, abi: BOUNTY_ABI, functionName: "pot", args: [k] });
    },
  };
  return agent;
}
