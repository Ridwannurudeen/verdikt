...
function readEnv() {
    ...
    return {
        ...
        court: COURT_ADDRESS,
        startBlock: START_BLOCK ? BigInt(START_BLOCK) : 0n,
    };
}
...
async function main() {
    ...
    const [reached, appealed, finalized] = await Promise.all([
        scanEvent(publicClient, cfg.court, EVENT("VerdictReached"), cfg.startBlock, latest),
        scanEvent(publicClient, cfg.court, EVENT("Appealed"), cfg.startBlock, latest),
        scanEvent(publicClient, cfg.court, EVENT("CaseFinalized"), cfg.startBlock, latest),
    ]);
    ...
    for (const row of table.values()) {
        try {
            ...
            console.error(`  getCase(${row.caseId}) failed: ${err.shortMessage ?? err.message}`);
        }
    }

    const rulings = [...table.values()].sort(
        (a, b) => Number(BigInt(a.caseId) - BigInt(b.caseId)),
    );
    ...
    const hist = {};
    for (const r of rulings) hist[r.verdict ?? "—"] = (hist[r.verdict ?? "—"] ?? 0) + 1;
    console.log(
        `\nverdict histogram: ${Object.entries(hist).map(([k, v]) => `${k}=${v}`).join("  ")}`,
    );
