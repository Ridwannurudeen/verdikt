...
function readEnv() {
  ...
  try {
    const url = new URL(SHANNON_RPC);
    if (url.protocol !== "https:" && url.protocol !== "http:") {
      die(2, `SHANNON_RPC must be http(s); got ${url.protocol}`);
    }
  } catch {
    die(2, `SHANNON_RPC is not a valid URL: ${SHANNON_RPC}`);
  }
...
async function main() {
  ...
  while (Date.now() < deadline) {
    ...
    const elapsed = Math.round(
      (POLL_TIMEOUT_MS - (deadline - Date.now())) / 1000,
    );
    console.log(
